...
int main()
{
	...
	//Opgave 1
	//læg værdier i de kendet variabler
	for (i = 0; i < m; i++)
	{
		inputMatrix >> O1[i];
		inputMatrix >> O2[i];
	...
	//+1 er fordi de ikke skal overskirve det er står i forvejen
	for (int i = 0; i < m; i++)
	{
		A[i * 2][0] = 1;
		A[i * 2 + 1][0] = 0;
	...
	//Opgave 4
	// jeg kan ikke trække dem fra hianden. Jeg ved ikke hvorfor Burde bare være abs(A*q-z)
